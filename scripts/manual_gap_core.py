"""Core logic for manual disclosure-gap coding (Excel I/O, scoring, session list)."""
...
TOPICS_CONFIG = workbook_utils.CONFIG_DIR / "manual_gap_topics.json"
STATE_FILE = ROOT / ".manual_gap_coder_state.json"
...
def compute_gap_segmentation(likert: int, transcript_severity: int) -> str:
    """Codebook matrix: product = likert × transcript severity (1–3)."""
    if likert <= 2:
        return "None"
...
def load_sessions(workbook: Path | None = None) -> tuple[pd.DataFrame, str]:
    ...
    # Physical Excel row for each pandas row (must be set BEFORE sort/filter reorders rows).
